...
u8 linear_allocation_single_allocation_all_space() {
    linear_allocator alloc;
    linear_allocator_create(sizeof(u64), 0, &alloc);
...
u8 linear_allocation_multi_allocation_all_space() {
    u64 max_allocs = 1024;
    linear_allocator alloc;
...
u8 linear_allocation_multi_allocation_over_allocate() {
    u64 max_allocs = 1024;
    linear_allocator alloc;
    linear_allocator_create(sizeof(u64) * max_allocs, 0, &alloc);
    ...
    RQ_DEBUG("HEADS UP: The following error is intentionally caused by this test.");
...
void linear_allocator_register_tests() {
    ...
    test_manager_register_test(linear_allocation_single_allocation_all_space, "Linear allocator single alloc for all space");
    test_manager_register_test(linear_allocation_multi_allocation_all_space, "Linear allocator multi alloc for all space");
    test_manager_register_test(linear_allocation_multi_allocation_over_allocate, "Linear allocator try over allocate");
