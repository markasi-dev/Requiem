...
// Windows platform layer.
    ...
    b8 platform_startup(
        ...
        wc.cbClsExtra = 0;  // Old legacy feature we dont need.
        wc.cbWndExtra = 0;  // Another old legacy feature.
        wc.hInstance = state->h_instance;
        wc.hIcon = icon;
        wc.hCursor = LoadCursor(NULL, IDC_ARROW);   // NULL; // Manage the cursor manually
        wc.hbrBackground = NULL;    // Transparent
        wc.lpszClassName = "requiem_window_class";

        // Throw error if window registration failed.
        ...
        u32 window_ex_style = WS_EX_APPWINDOW; // Makes sure we show in taskbar.
   ...
   void platform_get_required_extension_names(const char ***names_darray) {
    ...
    b8 platform_create_vulkan_surface(platform_state *plat_state, vulkan_context *context) {
        ...
        if (result != VK_SUCCESS) {
            RQ_FATAL("Vulkan surface creation failed.");
            return FALSE;
        }

        RQ_DEBUG("Surface handle immediately after creation: %p", (void*)context->surface);
        return TRUE;
    }
