...
#define VULKAN_USE_PLATFORM_XCB_KHR
...
// Key translation
keycodes translate_keycode(u32 x_keycode);
    ...
    while ((event = xcb_poll_for_event(state->connection)) != NULL) {
        ...
        switch(event->response_type & ~0x80) {
            ...
            case XCB_KEY_RELEASE: {
                ...
                keycodes key = translate_keycode(key_sym);

                // Finally pass to the input subsystem.
...
b8 platform_create_vulkan_surface(platform_state *plat_state, vulkan_context* context) {
    ...
    if (result != VK_SUCCESS) {
        RQ_FATAL("Vulkan surface failed to be created.");
        return FALSE;
    }
