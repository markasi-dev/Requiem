...
b8 vulkan_swapchain_acquire_next_image_index(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        // Do NOT recreate here — let the backend handle it via generation check.
        return FALSE;
...
void vulkan_swapchain_present(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        // Signal the backend to recreate next frame via generation check.
        // Do NOT recreate here and do NOT touch current_frame here.
        context->framebuffer_size_generation++;
    } else if (result != VK_SUCCESS) {
        RQ_FATAL("Failed to present swapchain image!");
    }

    // NOTE: current_frame is advanced by the backend (vulkan_renderer_backend_end_frame),
    // NOT here. Advancing it in both places was causing double-increment and sync corruption.
}
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    if (!found) {
        RQ_WARN("Preferred swapchain surface format not found — falling back to first available.");
        swapchain->image_format = context->device.swapchain_support.formats[0];
    }

    // Choose present mode; prefer mailbox (triple-buffer) over FIFO (vsync).
    ...
    // Re-query swapchain support to get fresh capabilities.
    vulkan_device_query_swapchain_support(
        context->device.physical_device,
        context->surface,
        &context->device.swapchain_support);

    // Resolve swapchain extent.
    ...
    // Request one more image than the minimum to avoid stalling on driver work.
    ...
    // Reset frame index on every (re)create.
    ...
    if (!swapchain->images) {
        swapchain->images = (VkImage*)rq_allocate(
            sizeof(VkImage) * swapchain->image_count,
            MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views) {
        swapchain->views = (VkImageView*)rq_allocate(
    ...
    if (!vulkan_device_detect_depth_format(&context->device)) {
        ...
        RQ_FATAL("Failed to find a supported depth format!");
    }
