    ...
    if (!fence->is_signaled) {
        ...
        switch(result) {
            case VK_SUCCESS:
                ...
                return TRUE;
            case VK_TIMEOUT:
                RQ_WARN("vk_fence_wait - Timed out");
                break;
            case VK_ERROR_DEVICE_LOST:
                RQ_ERROR("vk_fence_wait - VK_ERROR_DEVICE_LOST");
                break;
            case VK_ERROR_OUT_OF_HOST_MEMORY:
                RQ_ERROR("vk_fence_wait - VK_ERROR_OUT_OF_HOST_MEMORY");
                break;
            case VK_ERROR_OUT_OF_DEVICE_MEMORY:
                RQ_ERROR("vk_fence_wait - VK_ERROR_OUT_OF_DEVICE_MEMORY");
                break;
            default:
                RQ_ERROR("vk_fence_wait - An unknown error has occured!");
                break;
        }
    } else {
        // If already signalled, do NOT wait
        return TRUE;
    }
