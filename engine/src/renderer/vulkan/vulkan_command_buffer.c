...
void vulkan_command_buffer_allocate(
    ...
    rq_zero_memory(out_command_buffer, sizeof(vulkan_command_buffer));
    ...
    RQ_DEBUG("Vulkan command buffers created.");
}
...
void vulkan_command_buffer_begin(
    ...
    command_buffer->state = COMMAND_BUFFER_STATE_RECORDING; // mom get the camera BOIIIIIIII (ill shut the fuck up now).
}
